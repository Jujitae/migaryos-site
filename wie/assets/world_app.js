...
(function (root) {
  ...
  const ARTIFACT_LABELS={world_model:'세계 모델',scenario:'조건부 시나리오',learning_comparison:'학습 비교',outcome_tracking:'결과·가상 추적',evidence_coverage:'근거·커버리지'};
  const ARTIFACT_STATUS={PUBLIC:'공개 연결',PRIVATE_ONLY:'Founder 전용',MISSING:'현재 입력 없음'};
  const ARTIFACT_AVAILABILITY={CONNECTED:'현재 경로에서 확인 가능',UNAVAILABLE:'현재 공개 projection을 확인할 수 없음',NOT_PUBLISHED:'공개 발행하지 않음',NOT_PRESENT:'현재 세대에 없음'};
  ...
  function createApp({document:d,fetchImpl,storage=null,location={hash:''},history=root.history,eventTarget=root,founder=false,now=()=>Date.now(),setTimer=setTimeout,clearTimer=clearTimeout}={}) {
    ...
    function renderArtifacts(){clear('world-artifacts');const host=$('world-artifacts'),items=arr(view?.payload?.artifacts?.items);if(!host||!items.length)return;host.append(el('h2','현재 연결된 WIE 자산'));const list=el('ul','', 'artifact-status-list');for(const item of items){const label=ARTIFACT_LABELS[item.id]||item.id||'자산';const state=ARTIFACT_STATUS[item.status]||item.status||'미확인';const detail=ARTIFACT_AVAILABILITY[item.availability]||item.reason||'';list.append(el('li',label+' · '+state+(detail?' · '+detail:''),'artifact-status'));}host.append(list,el('p','공개된 예측·결과·근거와 Founder 전용 시나리오·학습 비교를 구분합니다. predictive_skill은 별도 평가 없이는 확립으로 표시하지 않습니다.','muted'));}
    function render(includeWorkspace=true){renderArtifacts();renderScope();renderCards();renderMap();renderRecords();if(includeWorkspace)renderWorkspace();searchBox?.update();atlas?.render();}
    function clearEvidence(preserveWorkspace=false,reason='unavailable'){evidenceState=reason;evidenceDeadline=null;searchBox?.reset();clear('decision-content');view=null;selected=null;chatId++;clearTimer(adaptiveTimer);adaptiveLensActive=false;for(const id of ['adaptive-content','world-artifacts','world-scope','change-cards','world-map','map-relations','record-list','lens-content','founder-hypotheses','question-list','review-history','source-health','source-categories'])clear(id);if($('record-count'))$('record-count').textContent='';if($('more-records'))$('more-records').hidden=true;if($('founder-private'))$('founder-private').hidden=true;if(isAtlas&&!destroyed)renderRecords();if(!preserveWorkspace)renderWorkspace();atlas?.render();}
